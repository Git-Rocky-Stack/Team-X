...
export function buildTeamXApi(ipc: IpcRendererLike): TeamXApi {
  return {
    ...
    agentImprovement: {
      list: (companyId: string) =>
        ipc.invoke(CHANNELS.agentImprovementList, {
          companyId,
        }) as Promise<AgentImprovementSnapshot>,
      run: (req: RunAgentImprovementRequest) =>
        ipc.invoke(CHANNELS.agentImprovementRun, req) as Promise<AgentImprovementRunResult>,
    },
    routines: {
      list: (companyId: string) =>
