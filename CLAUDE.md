...
**Phase 5 (Intelligence Layer) — in progress.** M28 (RAG foundation), M29 (RAG into agent turns), M30 (NLU engine + command palette), M31 (agentic loop — read-side), M32 (task planner — write-side), and **M33 (Copilot Service — periodic analyzer + proactive insights + ask-the-copilot)** all complete, **including the two deferred M33 follow-ups F3 (`CopilotEventWindow.clear` wired through the new `companies.archive` IPC) + F4 (post-restore `system-agent` / `system-copilot` bootstrap on `backup.restore`)** (2026-04-18). **M34 (Copilot UI — sidebar panel + dashboard widget + `Cmd+Shift+K`) is next.** 1114 unit tests + 9 E2E specs (10 Playwright cases) passing.
