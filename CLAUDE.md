...
**Phase 5 (Intelligence Layer) — complete. All 8 milestones shipped (M28 → M35). v1.1.0.** M28 (RAG foundation), M29 (RAG into agent turns), M30 (NLU engine + command palette), M31 (agentic loop — read-side), M32 (task planner — write-side), M33 (Copilot Service — periodic analyzer + proactive insights + ask-the-copilot) **plus** the two deferred M33 follow-ups F3 (`CopilotEventWindow.clear` wired through the new `companies.archive` IPC) + F4 (post-restore `system-agent` / `system-copilot` bootstrap on `backup.restore`), **M34 (Copilot UI — sidebar panel + dashboard widget + `Cmd+Shift+K`)**, and **M35 (Demo + Hardening — performance defaults audit + cross-milestone integration E2E + audit-view chips + retrospective + demo walkthrough + README/CHANGELOG reconciliation + v1.0.0 → v1.1.0 version bump + flaky-test audit + Phase 5 COMPLETE milestone marker)** all complete (2026-04-20). 1169 unit tests + 11 E2E specs (12 Playwright cases) passing. **Phase 6 scope TBD.**

**Phase 5.5 hotfix (Role-pack reconciliation + signing) — complete (2026-04-17).** Pre-M36-T2 audit by Rocky surfaced three drift issues: (1) only 37 role.md files on disk vs the 55+2=57 the M8 status block claimed, (2) `pack.json` still at v0.1.0 vs v1.0.0 the plan claimed, (3) no `pack.sig` in repo + no load-time signature verification. Root cause: M8 shipped 20 of its roles to a `worktree-phase-2-the-org` branch that **was never merged into main**; the M8 self-incriminating commit message even said "Combined with 22 roles on the phase-2-the-org branch" — author assumed the branch was already merged, but it wasn't. M9 → M35 ran on main with 35 user roles + 2 system roles (37 total). Fix shipped: cherry-picked the 20 stranded role.md files from `worktree-phase-2-the-org` onto main (post-harmonization state); extended `packages/role-schema/src/pack-signature.ts` with a real directory-tree signing surface (`canonicalTreeHash` deterministic over sorted POSIX-relative paths + per-file SHA-256, `signPackDirectory`/`verifyPackDirectory` Ed25519 with JSON envelope including `publicKeyFingerprint` + `treeHash` + `signedAt` for diagnostic value, +22 unit tests covering tampering / addition / deletion / rename / wrong-key / missing-sig / malformed-envelope); generated the official Ed25519 keypair via NEW `scripts/generate-pack-key.mjs` (private key in `.secrets/` gitignored, mode 0600; public key baked into `apps/desktop/src/main/services/role-loader.ts` as `STRATEGIA_OFFICIAL_PUBLIC_KEY` so the chain of trust is the git history of source code); bumped `pack.json` 0.1.0 → 1.0.0 + added `signed: true` + `publicKeyFingerprint`; signed the official pack via NEW `scripts/sign-pack.mjs` (committed `pack.sig`); wired load-time verification into `role-loader.ts` (NEW `verifyMode: 'strict' | 'warn' | 'off'` deps field with platform-aware default in the composition root — `strict` in packaged production, `warn` in dev so Rocky can edit role.md without re-signing on every save, `off` in test mode); NEW `pnpm sign:pack` + `pnpm sign:pack:keygen` scripts for the resign-after-edit workflow; NEW `scripts/count-roles.mjs` + `scripts/verify-pack-end-to-end.mjs` diagnostic tools. M8 line below preserved as written for historical accuracy — the truth lives here. Loader now indexes the full 57-role catalog (Officer 5 + Senior Mgmt 7 + Management 8 + Supervisor 5 + Lead 5 + IC 25 + System 2) with strict-mode signature verification passing. **Unblocks M36 T2.**
...
- **M34 (Copilot UI — sidebar panel + dashboard widget + `Cmd+Shift+K`) — complete (2026-04-18, all 11 tasks shipped):** Renderer-only milestone that consumes the M33 `copilot.*` IPC surface — **zero new IPC channels, zero new bus events, zero new providers**. Surfaces proactive insights in a right-side toggleable sidebar panel plus a compact top-3 dashboard widget, wires `Cmd+Shift+K` (Ctrl+Shift+K on Windows/Linux) as the global toggle, and hands free-form questions off to the existing M31 step-transcript layout with zero duplicated wire code. **T1** — Biome `pnpm lint:fix` format-only sweep on four pre-existing files (`apps/desktop/e2e/copilot-service.spec.ts`, `apps/desktop/src/main/ipc/handlers.ts`, `apps/desktop/src/main/services/backup.test.ts`, `.loki/queue/current-task.json`), clears the lint baseline before implementation begins. **T2** — `docs/plans/2026-04-18-team-x-phase-5-m34-copilot-ui.md` implementation plan (11 tasks, non-goals, architectural decisions for state location / bus invalidation / ask-input handoff / action dispatch / shortcut collision). **T3** — NEW `apps/desktop/src/renderer/src/hooks/use-copilot.ts` — four React Query hooks (`useCopilotInsights` / `useDismissCopilotInsight` / `useAskCopilot` / `useCopilotConfigure`). Single `events.onDashboard` listener inside `useCopilotInsights` fires `invalidateQueries({ queryKey: ['copilot-insights', companyId] })` on `copilot.insight` / `copilot.dismissed` / `copilot.expired` per architectural invariant #11. Dismiss is optimistic (`onMutate` filters the row out of the cache) with `onSettled` reconciliation. **T4** — NEW `apps/desktop/src/renderer/src/features/copilot/copilot-insight-card.tsx` — severity-coded `<li>` with category icon map (operational=Activity, cost=DollarSign, org=Users, workflow=GitBranch, anomaly=AlertTriangle), severity colour stripe + badge (critical=red, warning=amber, info=blue — `bg-*-950/60` on dark theme passes WCAG AA), optional action button dispatching `command.execute` with `actorId: 'copilot'`, dismiss X with `aria-label`. `data-copilot-insight-id` is the stable E2E selector surface (matches M31's `data-step-kind` pattern). **T5** — NEW `copilot-sidebar.tsx` — Radix `Sheet` panel (side=right, focus trap + Esc dismissal + `role=dialog` all free). Feed sorted `critical > warning > info` then newest-first within bucket; empty / loading / error states first-class. Ask textarea pinned to the bottom with Cmd/Ctrl+Enter submit (mirrors `features/chat/composer.tsx` keymap); on success calls `useAppStore.openThread({ isCopilotThread: true })` so the chat drawer auto-renders the M31 step-transcript read-only layout. **T6** — NEW `copilot-dashboard-widget.tsx` — top-3 cap with `(View all (N) / Open sidebar)` link toggling `copilotSidebarOpen` in the shared Zustand slice. Composed into the dashboard Cards subview via `renderDashboard()`. **T7** — `copilotSidebarOpen: boolean` + `setCopilotSidebarOpen(open)` added to `useAppStore` (`apps/desktop/src/renderer/src/store/app-store.ts`), single source of truth for all three entry points. `App.tsx` keyboard handler extended — same listener dispatches `Cmd+K` → palette toggle and `Cmd+Shift+K` → sidebar toggle based on `event.shiftKey`. Data-attribute bypass (`[data-copilot-sidebar-root]`) so the sidebar's own sheet (also `role=dialog`) does not swallow the toggle. Sparkles toolbar button (`data-copilot-toolbar-toggle`, `aria-pressed`, `aria-label="Toggle Copilot sidebar (Cmd+Shift+K)"`) added to the right side of `top-bar.tsx`. TopBar badge bumped `Phase 4` → `Phase 5`. **T8** — Action dispatch pipeline (wired inline inside T4 through `useCommandExecute.mutateAsync` — no separate module needed). Entities parsed via `parseActionEntities` (see T9). `confirmed: false` — destructive and write-side gates fire inside `CommandService` (M30 T5, M32 T5), never silent-bypassed from the copilot card. **T9** — NEW `copilot-helpers.{ts,test.ts}` — pure helpers extracted for testability (no DOM, no IPC, no React, matches M32 T6 `step-card-narrow.ts` pattern). `sortBySeverity` (severity rank + tie-break by `createdAt` descending, non-mutating), `parseActionEntities` (defensive JSON parse — rejects arrays / primitives / null / non-string values per `IpcExecuteRequest.entities: Record<string, string>` contract), `pickDashboardTopN` (cap + `hasMore` + `total` projection). 16 unit tests. **1130 unit tests** (1114 → +16), 99 test files green. **T10** — NEW `apps/desktop/e2e/copilot-ui.spec.ts` — full round-trip: seed ticket (create + close) → `copilot.configure` manual tick → click Sparkles toolbar → sidebar mounts + insight card visible via `data-copilot-insight-id` → active-count badge reads "1 active" → click dismiss X → card leaves DOM + `copilot.insights` IPC returns zero active → empty state renders → fill ask textarea with `__ECHO_AGENT__:` sentinel + click submit → sidebar closes + system-copilot thread resolved via `chat.listThreads` with `isSystemAgent=true` → regression guards on the M30 destructive gate ("Confirm destructive action") + M32 T5 write-side gate ("Confirm write-side agentic run") both absent. Spec runtime 2.0s. **E2E 10 specs / 11 cases green in 27.5s** (full suite re-ran clean after `Phase 4` → `Phase 5` badge assertion update across `smoke.spec.ts` / `ticket-flow.spec.ts` / `meeting-flow.spec.ts` / `vault-backup.spec.ts`). Accessibility refinements inside this milestone: insight card uses semantic `<li>`, feed uses `<ul>`, loading / error / empty states sit as siblings of the `<ul>` so the list never contains non-`<li>` children (WCAG-valid markup); dashboard widget wraps cards in `<ul data-copilot-widget-list="">` to mirror the sidebar contract; severity colour never the sole meaning carrier — text badge always paired. **T11** — Documentation (CLAUDE.md / CHANGELOG / README / Phase 5 design §9 / `docs/user-guide/copilot-ui.md` NEW / `docs/user-guide/keyboard-shortcuts.md` update) + full verification (Node rebuild → vitest 1130 → Electron rebuild → Playwright 10 specs / 11 cases) + milestone marker.

- **M35 (Demo + Hardening) — complete (2026-04-20, all 11 tasks shipped):** Phase 5 exit-gate milestone. Hardens everything M28–M34 shipped and lands v1.1.0. **T0** — plan doc (`docs/plans/2026-04-19-team-x-phase-5-m35-demo-hardening.md`, 186 lines, mirrors M33/M34 structural template). **T1** — performance defaults audit: measured Ollama + llama3.1:8b (Q4_K_M) against the analyzer-shaped prompt (67 bounded events → 2288 char / 734 prompt tokens); cold 208s, warm 66s per tick; all 10 settings clamps HELD on measurement evidence (66s warm vs 120s `agentic_timeout_ms` = 0.55× utilisation; 5-min copilot cadence vs 66s tick = 4.5× headroom; 8000-token budget vs 1600 projected = 4× headroom). +4 unit → 1134. **T2** — cross-milestone integration E2E (`phase-5-integration.spec.ts`): single Electron boot exercises RAG index → command palette → `complex_request` → agentic-loop tool call → grounded answer → write-side decompose → copilot analyzer tick → insight card → dismiss round-trip. +1 E2E → 11 spec files / 12 Playwright cases. **T3** — audit-view chips for the 18 Phase 5 event types (`rag.index.*`, `agent.step`, `agentic.*`, `plan.*`, `task.*`, `review.*`, `copilot.*`, `command.executed`) with 200-char payload summary helpers extracted into `audit-event-chip-helpers.ts` for source-string-audit testability. +28 unit → 1162. **T4** — `docs/plans/2026-04-19-team-x-phase-5-retrospective.md` (271 lines, locked six-section structure: delivery arc / what paid off / what cost time / deferred scope / Phase 6 seeds / appendix). Every future phase retrospective MUST match this structure. **T5** — `docs/demo/phase-5-walkthrough.md` + 5 scenario docs under `docs/demo/scenarios/` (hire-a-CEO / ticket-lifecycle / one-click-all-hands / ask-copilot-grounded-answer / decompose-and-surface). 795 insertions across 6 new files. **T6** — README + user-guide reconciliation sweep (tests badge 1130 → 1162, E2E 10 → 11 specs, Phase 5 status block flipped from "in progress" to "complete" in every user-facing doc). **T7** — CHANGELOG promotion: accumulated `[Unreleased]` entries (M30 + M31 + M32 + M33 + M33 F3/F4 + M34) promoted into `[1.1.0] — 2026-04-20` under new "Phase 5 — Intelligence Layer" narrative header; `[Unreleased]` reset to six-category scaffold; version-link refs appended. **T8** — version bump 1.0.0 → 1.1.0 across 7 `package.json` files (workspace root + `apps/desktop` + 5 `packages/*`) + `top-bar.test.tsx` pinning the literal `Phase 5` badge string (so an accidental `Phase 6` bump is caught by CI before Phase 6 work officially begins). +2 unit → 1164. **T9** — flaky-test audit + stable-selector sweep: NEW `apps/desktop/src/e2e-regression-guards.test.ts` with 2 vitest guards (no spec uses `waitForTimeout(N > 100)` + every spec uses at least one `[data-*]` attribute locator); fixed `rag-flow.spec.ts` `waitForTimeout(500)` → `expect.poll` on `teamx.rag.stats.embeddingCount > 0`; added `[data-copilot-toolbar-toggle]` stable-selector anchor to 6 specs. +2 unit → 1166. **T10** — docs + verification + Phase 5 COMPLETE milestone marker: flipped the CLAUDE.md Phase 5 status line + subheader; flipped Phase 5 design doc §9 M35 row 📋 Planned → ✅ Complete; appended Phase 5 COMPLETE section to `.loki/CONTINUITY.md`; NEW `docs/user-guide/demo-walkthrough.md` (promoted external-facing copy of `docs/demo/phase-5-walkthrough.md`); +3 unit release-marker tests (`phase-5-complete-marker.test.ts` — source-string audit per M35 T3/T8/T9 convention, pinning the CLAUDE.md Phase 5 COMPLETE literal + design doc M35 ✅ Complete row + CONTINUITY.md Phase 5 COMPLETE header). 1169 unit / 11 E2E specs (12 Playwright cases). Atomic + ledger commit cadence preserved throughout. v1.1.0 git tag on the T10 ledger commit. **Phase 5 ships. Next session opens Phase 6 T0 (scope TBD).**
