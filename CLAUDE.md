...
**Phase 5 (Intelligence Layer) — in progress.** M28 (RAG foundation), M29 (RAG into agent turns), M30 (NLU engine + command palette), M31 (agentic loop — read-side), M32 (task planner — write-side), M33 (Copilot Service — periodic analyzer + proactive insights + ask-the-copilot) **plus** the two deferred M33 follow-ups F3 (`CopilotEventWindow.clear` wired through the new `companies.archive` IPC) + F4 (post-restore `system-agent` / `system-copilot` bootstrap on `backup.restore`), and **M34 (Copilot UI — sidebar panel + dashboard widget + `Cmd+Shift+K`)** all complete (2026-04-18). **M35 (Demo + Hardening) is next.** 1130 unit tests + 10 E2E specs (11 Playwright cases) passing.
